...
export class ConnectHandler {
    ...
    async onLogin(msg: any, session: FrontendSession) {
        try {
            ...
            if (req == null) {
                return AppProtocol.response(ProConnect.Login.Response, false, '请求参数错误');
            }
            return await ConnectVerifyController.getInstance().onLogin(session, req);
        } catch (error) {
            ConnectLogger.error(error);
        }
        return AppProtocol.response(ProConnect.Login.Response, false, '请求失败');
    }
    ...
    async onRegister(msg: any, session: FrontendSession) {
        try {
            ...
            if (req == null) {
                return AppProtocol.response(ProConnect.Register.Response, false, '请求参数错误');
            }
            return await ConnectVerifyController.getInstance().onRegister(session, req);
        } catch (error) {
            ConnectLogger.error(error);
        }
        return AppProtocol.response(ProConnect.Register.Response, false, '请求失败');
    }
